function SL=SL_single_twosided(dataA,dataB,w1,w2,embedding,sampling_delay,p_ref,stps,delays)
% revised by zifang zhao @ 2012-12-28 输入的数据起始点为-w2到w2+max(stp)+max(delay)
% renamed to SL_single_twosided from SL_single 改为双边计算 2012-12-27 
% by zifang zhao
...
stps=stps+1;
% delay_0=min(delays);
% delays=delays+1-delay_0;
data_trim_len=2*w2+max(stps) ;
sel_A=(1:data_trim_len);
sel_B=sel_A;
for dly_idx=1:length(delays)
    delay=delays(dly_idx);
%     data_trim_len=length(dataA)-delay_len;
    dataA_cal=dataA(sel_A);   %对B进行延时
    dataB_cal=dataB(delay+sel_B);
%     data_len=size(dataA_cal,2);
%     if data_len>w2+max(stps)+(embedding-1)*sampling_delay %判断数据长度是否小于可计算的最小长度
        %% 生成向量矩阵
        vecA=zeros(embedding,max(stps)+2*w2-(embedding-1)*sampling_delay);
        vecB=zeros(embedding,max(stps)+2*w2-(embedding-1)*sampling_delay);
        
        for ebd=1:embedding %根据向量维度生成不同时移的矩阵，再进行拼合
            vecA(ebd,:)=dataA_cal(1+(ebd-1)*sampling_delay:(ebd-1)*sampling_delay+2*w2-(embedding-1)*sampling_delay+max(stps));
            vecB(ebd,:)=dataB_cal(1+(ebd-1)*sampling_delay:(ebd-1)*sampling_delay+2*w2-(embedding-1)*sampling_delay+max(stps));
        end
        
        %% 计算每一组向量和参考向量间的距离
%       vref_range=[1+w2:step:cal_len-w2]; %参考向量的计算区间
        vref_range=stps+w2;    %参考向量的计算区间
...
%         vec_R_B=zeros(length(vref_range),vec_num);
        ...
        for ref_idx=1:length(vref_range)
            ...
            tim=[ref_tim-w2:ref_tim-w1-(embedding-1)*sampling_delay,ref_tim+w1:ref_tim+w2-(embedding-1)*sampling_delay];  %双侧计算
