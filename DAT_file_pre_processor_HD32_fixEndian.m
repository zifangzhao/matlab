function DAT_file_pre_processor_HD32_fixEndian(filename)
if nargin<1
    [f,p]=uigetfile('*.dat');
    filename=[p f];
    cd(p);
...
while start*Nchan*2<fb && start<time_limit
    [a,fb]=readmulti_frank(filename,Nchan,chans,start,start+bulk,'uint16');
    x=dec2hex(a);
    y=x;
    y(:,1:2)=x(:,3:4);
    y(:,3:4)=x(:,1:2);
    filter_temp=hex2dec(y);
