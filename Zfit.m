...
%                         % "pure" capacitor=0.1 nF, a second resistor =1.5 kOhms
...
% 
% EXAMPLE 4:
% Now, fit the data with another model whose the string has 3 R//C in series
%                         circuit='s(p(R1,C1),p(R1,C1),p(R1,C1))';     % describes the circuit shape
%                         indexes=[];
%                         param=[2e4, 1e-10, 2e4, 1e-9, 2e4, 1e-8];
%                         [p,zbest,fval]=Zfit([freq,zexp],'z',circuit,param,indexes,'fitNP');
%                         after the Matlab message :
%                                 "Exiting: Maximum number of function evaluations has been exceeded
%                                 - increase MaxFunEvals option.
%                                 Current function value: 1798684872.191325"
%                         retry the fitting with the values "p" sent by the function :
%                         [p,zbest,fval]=Zfit([freq,zexp],'z',circuit,p,indexes,'fitNP');
%                         redo while the fitting is improving (fval decreases to a minimum).
%
% HINTS:
