function general_mat_sl_multidelay_gpu(searchkeyword,system_fs,LP,HS,n_rec,p_ref,stps,delays,max_steps,n_level)
% modified from general_sl_multidelay_gpu by zifang zhao @ 2013-10-18
% 设计为读取矩阵模式
...
calAll=0;
s2=[];
while isempty(matfile)
    button=questdlg('Maybe current folder is not the container folder,reselect?','Please check!');
    switch button
        case 'Yes'
...
str={matfile.name};
[s,~] = listdlg('PromptString','Select files to run',...
...
for file_idx=1:length(matfile)
    ...
    if isempty(filename);
       warndlg('Please check for excel file!','!! Warning !!')
       return;
    end
...
%             if isempty(searchkeyword)
                fname=[matfile(file_idx).name(1:end-4-length(searchkeyword)) '_sl_' fieldname freq '.mat'];
%             else
%                 fname=[matfile(file_idx).name(1:end-4-length(searchkeyword)) '_sl_' fieldname freq '.mat'];
%             end
            %% data_reorganize,降采样
...
%                     cwd=pwd;
%                     cd('c:\');
                    rawdata_inp=load(matfile(file_idx).name);
%                     system(['del ' mat_name]);
%                     cd(cwd);
                    fread=1;
                end
...
%                 chans=channelpick(fieldnames(rawdata),'elec');
                fld=fieldnames(rawdata_inp);
                if isempty(s2)
                [s2,v]=listdlg('PromptString','Select the LFP field','SelectionMode','single','ListString',fld);
                end
                rawdata=getfield(rawdata_inp,fld{s2});
                chans=1:size(rawdata,1);
%                 channel_all=cellfun(@(x) x.channel,rat_montage);%在这里增加通道的预筛选
...
%                     eval(['rawdata_valid{i} = resample(rawdata.elec' num2str(chans(i)) ',res_fs,system_fs);']);
                         rawdata_valid{i}=resample(rawdata(i,:),res_fs,system_fs);
                end
...
%                     if isempty(old_file)
                        %进行sl计算
                        SL=SL_LFP_twosided_gpu_mex(rawdata_valid,res_fs,starts,ends,chans,LP(freq_idx),HS(freq_idx),n_rec,p_ref,stps_fixed,delays_fixed,fname(1:end-4),n_level,1);
