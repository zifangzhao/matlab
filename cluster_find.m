function [cent]=cluster_find(data,min_area,max_ecc,min_I,min_percent,plot_on)
% revised by zifang zhao @ 2012-9-27 merge cent and mean of cluster into a
% (n,3) matrix
...
if ~isempty(s)
    ...
    for num=1:size(bbox,1);
        ...
        s_mean(num)=mean(reshape(data(bboxf(num,1):(bboxf(num,1)+bboxf(num,3)-1),bboxf(num,2):(bboxf(num,2)+bboxf(num,4)-1)),[],1));
    end
    cent=[cent s_mean];
end
hold off;
