...
%     try
        if isstruct(data)
            if length(data.data)>100
                stock.date=cellfun(@(x) datenum(x,'mm/dd/yyyy'),data.textdata(1:end-1-date_end_offset));
                stock.data=data.data(1:end-date_end_offset,1:end); %开 高 低 收 量 价
                ...
                days=round(linspace(1,7,3));
                predicted_values=cell(length(days),1);
                tag_fitted=cell(length(days),1);
                for d_idx=1:length(days)
                    ...
                    tag_fitted{d_idx}=feval(mdl,analysis_mat);
                    plot(1:length(tag),tag);
                    hold on;
                    plot((1:length(tag))+p_day,tag_fitted{d_idx},'r');
                    hold off;
%                     axis tight
                    xlim([length(tag)+max(days)-200 length(tag)+max(days)]);
                    title([f ' 预测' num2str(p_day) '天']);
                    predicted_values{d_idx}=tag_fitted{d_idx}(end-p_day:end);
                    %     legend('前复权日线','预测值');
                end
...
% evaluate_mat=cellfun(@(x) x(:,:),rate_predict,'UniformOutput',0);
% evaluate_mat=cat(2,evaluate_mat{:});
% evaluate_mat=mean(evaluate_mat,2)./std(evaluate_mat,[],2);
evaluate_mat=cellfun(@(x) mean(x,2),rate_predict,'uniformoutput',0);
ix=1:length(predicted);
% [~,ix]=sort(evaluate_mat{2},'descend');
