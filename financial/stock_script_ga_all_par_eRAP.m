...
function [stock predict_avg column selected_para] = stock_script_ga_all_par_eRAP(para_sel,days,fft_day,day_predict,gen,popu,tol,offset)
if nargin<8
...
while next_idx~=Inf
    current_idx=next_idx;
    [stock next_idx max_idx]=stock_read_v3(next_idx+1,batch_size,days+offset);
    sch=parcluster();
