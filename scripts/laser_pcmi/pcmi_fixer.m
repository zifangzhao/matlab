...
for idxd=1:length(collected_data)
    ...
    data_new{idxd}.name=collected_data{idxd}.name;
    data_new{idxd}.data.Ixys=cell(1,length(collected_data{idxd}.data.Ixys));
    data_new{idxd}.data.Iyxs=cell(1,length(collected_data{idxd}.data.Iyxs));
    data_new{idxd}.data.delay=cell(1,length(collected_data{idxd}.data.Ixys));
...
%         multiWaitbar('Trials:',trial/length(collected_data{idxd}.data.Ixys),'color',[0.2 0.75 0.37]);
        data_new{idxd}.data.Ixys{trial}=cell(length(channel_all));
        data_new{idxd}.data.Iyxs{trial}=cell(length(channel_all));
        data_new{idxd}.data.delay{trial}=cell(length(channel_all));
...
%                 multiWaitbar('channel2:',ch_b/length(collected_data{idxd}.channeldef.def),'color',[0.5 0.5 0.5]);
                ...
                if(isempty(Ixys_temp));
                    ...
                    data_new{idxd}.data.Ixys{trial}{x,y}=Ixys_temp;
                    data_new{idxd}.data.Iyxs{trial}{x,y}=Iyxs_temp;
                    data_new{idxd}.data.delay{trial}{x,y}=1:length(data_new{idxd}.data.Ixys{trial}{x,y});
                end
            end
        end
    end
