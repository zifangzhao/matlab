function ripple_waves_plot(f,HD32_B_map)
%% Loading gamma times
if nargin==0
f=uigetfile('ripple.mat');
end
if nargin<2
...
ripple=getfield(matfile,field{1});

gamma_start=ripple(:,1);
gamma_end=ripple(:,3);
...
tmp=filtfilt(Hd.sosMatrix,Hd.ScaleValues,Data(1,:,1));
tmp=0.7*tmp(end/2-500:end/2+500);
r=max(tmp)-min(tmp);
gamma_fil=cell(length(HD32_B_map),1);
sos=Hd.sosMatrix;
g=Hd.ScaleValues;
parfor chn=1:length(HD32_B_map)
...
for chn=1:length(HD32_B_map)
    ...
    plot(t,-r*chn+median(gamma_fil{HD32_B_map(chn)},2),'r')
end
axis tight
xlim([-100 100])
xlabel('Time(ms)')
