function DAT_SaveEvents(filename,det_mat,channelID,Eventname)
...
events.time = r(:);
for i = 1:3:3*n,
	events.description{i,1} = [Eventname ' start ' int2str(channelID)];
	events.description{i+1,1} = [Eventname ' peak ' int2str(channelID)];
	events.description{i+2,1} = [Eventname ' stop ' int2str(channelID)];
end
if ~isempty(filename)
    if ~ispc
        system(['rm ''' filename '''']);
