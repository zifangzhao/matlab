...
% Last Modified by GUIDE v2.5 19-Apr-2012 18:40:56
...
set(handles.text8,'String',['Current file: ' filename]);
test_result=m.test_result;
%显示可选子集
% fld_temp=fieldnames(test_result{1});
fld=fieldnames(test_result{1}.tests);
...
test_type=temp(get(handles.listbox4,'Value'));                  %检验种类 
temp=get(handles.listbox6,'String');
data_type=temp(get(handles.listbox6,'Value'));                   %测试结果分类
temp=get(handles.listbox7,'String');
sub_type=temp(get(handles.listbox7,'Value'));                   %检验结果子类

label_group=attr_extracter(get(handles.edit1,'String'),';'); %人工输入标签A,将输入标签分组

for idxg=1:length(label_group)
    labelA{idxg}=attr_extracter(label_group{idxg},' ')';
end
...
locAB=ones(1,size(map_attr_allA,2));
if ~isempty(label_group)
    for idxg=1:length(labelA)
        loc_temp=ones(1,size(map_attr_allA,2));
        for idx=1:length(labelA{idxg})
            if idx==1
                loc_temp=loc_temp&sum(cell2mat(cellfun(@(x) sum(strcmp(x,labelA{idxg}{idx})),map_attr_allA,'UniformOutput',0)));
            else
                loc_temp=loc_temp|sum(cell2mat(cellfun(@(x) sum(strcmp(x,labelA{idxg}{idx})),map_attr_allA,'UniformOutput',0)));
            end
        end
        locAB=locAB&loc_temp;
    end
end
locA=locAB&sum(cell2mat(cellfun(@(x) sum(strcmp(x,label_compare_A)),map_attr_allA,'UniformOutput',0)));

loc=cell(1,length(label_compare_B));
for idx1=1:length(label_compare_B)
    labelB_current=label_compare_B(idx1);
    locB=locAB;
...
for idx1=1:length(label_compare_B)
    eval(['temp=cell2mat(cellfun(@(x) x.tests.'  data_type{1} '.' test_type{1} '.' sub_type{1} ',test_result(loc{idx1}),''UniformOutput'',0));']);
...
global test_result
fldresult=get(handles.listbox6,'String');
fldresult_idx=get(handles.listbox6,'Value');
fld=get(handles.listbox4,'String');
fld_idx=get(handles.listbox4,'Value');
eval(['svalue=fieldnames(test_result{1}.tests.' fldresult{fldresult_idx} '.' fld{fld_idx} ');']);
set(handles.listbox7,'String',svalue);
...
eval(['stype=fieldnames(test_result{1}.tests.' fld{fld_idx} ');']);
set(handles.listbox4,'String',stype);
