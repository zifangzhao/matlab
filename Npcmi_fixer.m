function collected_data=Npcmi_fixer(rat_montage,collected_data)
%% 多延时pcmi重新整理，扩充channel
%created by zifang zhao@ 2012-6-23 based on the pcmi_fixer
channel_all=[];
...
for idxd=1:length(collected_data)
    ...
    for sub_data_idx=1:length(data_sub_name)
        ...
        for trial=1:length(data);
            ...
            temp=cell(chan_size);
            channel_in_mont=cellfun(@(x) x.channel,collected_data{idxd}.channeldef.region);
            for ch_a=1:length(channel_in_mont)
                %             multiWaitbar('Channel1:',ch_a/length(collected_data{idxd}.channeldef.def),'color',[0.6 0.3 0.9]);
                for ch_b=1:length(channel_in_mont)
                    %                 multiWaitbar('channel2:',ch_b/length(collected_data{idxd}.channeldef.def),'color',[0.5 0.5 0.5]);
                    sub_temp=data{trial}{ch_a,ch_b};
                    
                    if(isempty(sub_temp));
                    else
                        x=channel_in_mont(ch_a);
                        y=channel_in_mont(ch_b);
