...
UID=inputdlg('Enter the UID(Decimal):','UID');
UID=str2num(UID{1});
UID_hex=arrayfun(@(x) [dec2hex(x,8) ' '],UID,'uniformoutput',0);
...
xlsdata={uid_addr,UID(:),UID_hex{:},UID_encryp(:),[p f],datestr(now),key};
fh=fopen('c:\STM32_programming_record.csv','a');
fprintf(fh,'%s\n',[datestr(now),',',num2str(uid_addr),',',num2str(UID),',',UID_hex{:},',',num2str(UID_encryp),',',[p f],',',num2str(key),',','Offline']);
fclose(fh);
%xlswrite('C:\programming_record',xlsdata,'STM32F2F4');