function data = Phy_LoadCE32Data(filename,read_from_sample,read_to_sample)
getfilegui=0;
if nargin<1
    getfilegui=1;
else
...
if getfilegui == 1
    [f,p] = uigetfile({'amplifier.dat','Binary Ephys File (amplifier.dat)'});
    filename = [p f];
end
if nargin<2
    read_from_sample=0;
...
f_analog = strrep(filename,'amplifier','analogin');
f_digital = strrep(filename,'amplifier','digitalin');
data = [];
data.raw_data = readraw(filename,32,1:32,read_from_sample,read_to_sample);
data.analogin = readraw(f_analog,6,1:6,read_from_sample,read_to_sample,'uint16');
temp  =  readraw(f_digital,1,1,read_from_sample,read_to_sample,'uint16');
data.digitalin = dec2digits(temp,16);

function [eeg,fb] = readraw(fname,numchannel,chselect,read_start,read_until,precision,b_skip)
