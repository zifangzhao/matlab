...
ch_sel=ch_sel-1;
 ...
 for idx=1:length(ch_sel)
     ...
     [epsilon,sd,bad] = FindRipples(data_fil,'thresholds',threshold, 'durations', duration); 
     
     gamma_file = [p f(1:end-4)  num2str(ch_sel(idx) ) 'epsilon'];
     save (gamma_file, 'epsilon')
     spindle_events = [p f(1:end-4)  num2str(ch_sel(idx),'.e%02d')  '.evt'];
     SaveRippleEvents(spindle_events,epsilon,ch_sel(idx));
