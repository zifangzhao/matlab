function bands = CoherenceBands_frank (coherogram,frequencies,varargin)

%CoherenceBands - Determine running power in physiological bands.
%
%  USAGE
%
...
%  OUTPUT
%
%    bands.custom     coherence in the custom-set band
%    bands.delta      coherence in the delta band 
%    bands.theta      theta coherence
%    bands.spindles   spindle coherence
%    bands.lowGamma   low gamma coherence
%    bands.highGamma  high gamma power coherence
%    bands.ripples    ripple power coherence
%
%
%  SEE
%
...
alphaBins = frequencies >=alpha(1) & frequencies <=alpha(2);
betaBins = frequencies >= beta(1) & frequencies <= beta(2);
...
bands.alpha = mean(coherogram(alphaBins,:))';
bands.beta = mean(coherogram(betaBins,:))';
