...
function general_pcmi_laser(searchkeyword,bin,order,tau,delta)
% searchkeyword='raw';
...
for file_idx=1:length(matfile)
    ...
    chans=channelpick(fieldnames(rawdata),'unit2');
    %% data_reorganize
    for i = 1:length(chans)
        eval(['rawdata_valid{i} = rawdata.elec' num2str(chans(i)) '_unit2;']);
    end
