...
for idx=1:length(fc_all)
    ...
    fbase=[p_fig num2str(clu_id)  '_ALL_PSTH_' evt_tag];
    saveas(h,[fbase '.fig']);
    saveas(h,[fbase '.eps']);
    print(h,[fbase '.jpg'],'-djpeg');
...
%         n=hist(tim_hist,win(1):bin:win(2));
        ...
        bar(win(1):bin:win(2),spk_n{1,s_idx}/bin/length(evt_time),1,'EdgeColor','None','FaceColor','b');
        hold on;
        ...
        fbase=[p_fig num2str(clu_id) '_'  num2str(spk_list(s_idx)) '_PSTH_' evt_tag];
        saveas(h,[fbase '.fig']);
        saveas(h,[fbase '.eps']);
        print(h,[fbase  '.jpg'],'-djpeg');
    end
