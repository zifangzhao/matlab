...
//     double * p_min_num;
    ...
    char const * const errMsg = "Invalid input to MEX file.Startpoint and delay array must have the same increment. Startpoint must start from 0. Delay value must stays in 0+N*inc axis as startpoint do.";
...
//     }
    ...
    if((((int)stps[0]%inc)!=0)||(((int)delays[0]%inc)!=0)){
        mexErrMsgIdAndTxt(errId, errMsg);
    }
    
    Nvec=(int) (w2-w1-(embedding-1)*sampling_delay)*2;
    Nstep=Nstart+Ndelay+stps[0]/inc+delays[0]/inc;
...
//         mxGPUDestroyGPUArray(dist);
        ...
        SLGen<<<blocks_SL,Nvec>>>(p_SL,p_pattern,p_GVref,Nvec,Nstep,Nchn,Nstart,(int) delays[0]/inc);
