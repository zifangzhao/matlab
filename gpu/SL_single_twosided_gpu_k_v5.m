function SL=SL_single_twosided_gpu_k_v5(dataA,dataB,w1,w2,embedding,sampling_delay,p_ref,stps,delays,k)

% revised by zifang zhao @ 2013-11-18  add the support of shared memory 
% 注意截取数据时应从-w2到w2+max(stp)+max(delay)
% renamed to SL_single_twosided_gpu_k_v5 from SL_single_gpu_k_v5
% 改为双边计算2012-12-28 by zifang zhao
...
stps=stps+1;
delays=delays+1;
% max(stps)+2*w2-(embedding-1)*sampling_delay
vec_num=(w2-w1-(embedding-1)*sampling_delay)*2;%%如果修正tim，此处也需要修正
...
k.SharedMemorySize=0*vec_num*embedding;
stp_max=max(stps);
for dly_idx=1:length(delays)
    delay=delays(dly_idx);
    data_trim_len=2*w2+stp_max;
...
%     if data_len>w2+stp_max+(embedding-1)*sampling_delay %判断数据长度是否小于可计算的最小长度
        %% 生成向量矩阵
        vecA=zeros(embedding,stp_max+2*w2-(embedding-1)*sampling_delay);
        vecB=zeros(embedding,stp_max+2*w2-(embedding-1)*sampling_delay);
        
        for ebd=1:embedding %根据向量维度生成不同时移的矩阵，再进行拼合
            vecA(ebd,:)=dataA_cal(1+(ebd-1)*sampling_delay:(ebd-embedding)*sampling_delay+2*w2+stp_max);
            vecB(ebd,:)=dataB_cal(1+(ebd-1)*sampling_delay:(ebd-embedding)*sampling_delay+2*w2+stp_max);
        end
...
%         vref_range=stps;    %参考向量的计算区间
        ...
        vref_range=stps+w2;
        tim=[-w2:-w1-(embedding-1)*sampling_delay,w1:w2-(embedding-1)*sampling_delay];  %%考虑是否合理，未使用sampling_delay分隔
        distA=zeros(stp_len,vec_num);
        distB=zeros(stp_len,vec_num);
        distA=feval(k,distA,reshape(vecA,1,[]),int32(vref_range),int32(tim),int32(embedding));
%         distA=feval(k,distA,reshape(vec_zeroA,1,[]),int32(embedding));
        distA=gather(distA);
%         distB=feval(k,distB,reshape(vec_zeroB,1,[]),int32(embedding));
        distB=feval(k,distB,reshape(vecB,1,[]),int32(vref_range),int32(tim),int32(embedding));
        distB=gather(distB);
        %% 根据p_ref计算关键距离
