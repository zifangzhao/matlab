function SL_comp=SL_LFP_twosided_gpu(rawdata_valid,system_fs,starts,ends,chans,LP,HS,n_rec,p_ref,stps,delays,fname,n_level,proc_avg)
% revised by zifang zhao @ 2013-11-27 revised sampling_delay,+1
...
%% 参数变换
sampling_delay=1+floor(system_fs./HS/3);% L
            ...
            if proc_avg==0
                for m = 1:channels_len
                    for n = 1:channels_len
                        ...
                        t0=toc;
                        SL{trial}{m,n}=SL_single_twosided_gpu_k_v5(channels(m,:),channels(n,:),w1,w2,embedding,sampling_delay,p_ref,stps,delays,k);
                        t1=toc-t0;
