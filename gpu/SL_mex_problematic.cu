...
//      t=rawdata[chn_idx+temp_idx*Nchn];
    shared_data[temp_idx]=rawdata[chn_idx+temp_idx*Nchn]; //将1个channel的数据分配到共享内存中
    temp_idx+=BlockSize;
    __syncthreads();
}
...
if(temp_idx<Nvec*Nstep){ //因为theradsperblock是固定的，因此会有线程块并不参与后期计算，只参与共享内存
...
//计算所计算的向量在原始数据中的绝对索引
    ...
    for(int ebd=0;ebd<embedding;ebd++){
        len=shared_data[refe+ebd*sampling_delay]-shared_data[tim+ebd*sampling_delay];
        temp+=len*len;
...
//-----------------------calculating pattern---------------------------
    ...
    ref_idx=(int) temp_idx/Nvec; //this is vector idx
    tim_idx=temp_idx % Nvec;//this is step idx
    
    pattern[ref_idx+tim_idx*Nvec+chn_idx*Nvec*Nstep]=dist[ref_idx+tim_idx*Nvec+chn_idx*Nvec*Nstep]<=cdist[ref_idx+chn_idx*Nstep];
}
/*---------------------------------------Convert pattern into SL matrix-------------*/
void __global__ SLGen(double * SL, int * pattern,const double * vref,double Nvec,double Nstep,double Nchn,double Nstart,int dly_offset){
...
//     int y=blockIdx.y;// channel number B
    ...
    int B=pattern[(int)((stp+dly+dly_offset)+tim_idx*Nstep+y*Nvec*Nstep)];
    syn[2*tim_idx]=A&B;
    syn[1+2*tim_idx]=A+B;
    __syncthreads();
    if(tim_idx==0){
...
//     }
    ...
    Nstep=Nstart+Ndelay+delays[0]+stps[0];//N step startpoint plus N step delay and the bias of startpoint and the delay gap between two channels
...
//     int blocksPerGrid=Nchn*(1+Nvec*Nstep/threadsPerBlock);
    ...
    //这里需要计算出实际需要的step,需要step大于1
//     int stp_min=stps[0]; //在general_sl_multidelay_gpu中，已经修正为0，即起始时间一定为0
//     int delay_min=delays[0];
//     int stp_max=stps[Nstart];
//     int delay_max=delays[Ndelay];
    int inc=stps[1]-stps[0];
    for(int idx=0;idx<Nstep;idx++){
        p_vref[idx]=inc*idx+w2;
    }
...
//     p_Gtim=(double * )mxGPUGetData(Gtim); 
   ...
   GPUprctile<<<blocks_C,Nstep % (threadsPerBlock+1)>>>(p_dist,p_cdist,N_small,Nstep,Nvec); 
   ...
   SLGen<<<blocks_SL,Nvec>>>(p_SL,p_pattern,p_GVref,Nvec,Nstep,Nchn,Nstart,(int)delays[0]);
...
//     double * para_stp=mxGetPr(stp_out);
    double temp[]={Datalen,Nchn,stps[0],delays[0],inc,Nstep};
//     para_C[0]=Datalen;
//     para_C[1]=Nchn;
//     para_C[2]=embedding;
...
//     mxSetCell(plhs[0],0,mxGPUCreateMxArrayOnCPU(rawdata));  //testing by output to 1
    mxSetCell(plhs[0],0,mxGPUCreateMxArrayOnCPU(SL));  //testing by output to 1
    mxSetCell(plhs[0],1,para);
//     mxSetCell(plhs[0],2,ref_out);
//     mxSetCell(plhs[0],3,tim_out);
//     mxSetCell(plhs[0],4,mxGPUCreateMxArrayOnCPU(dist));  //testing by output to 1
//     mxSetCell(plhs[0],5,mxGPUCreateMxArrayOnCPU(cdist));  //testing by output to 1
//     mxSetCell(plhs[0],6,mxGPUCreateMxArrayOnCPU(pattern));  //testing by output to 1
