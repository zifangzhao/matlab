function SL_comp=SL_LFP_gpu(rawdata_valid,system_fs,starts,ends,chans,LP,HS,n_rec,p_ref,stps,delays)
...
w1_g=gpuArray(w1);
w2_g=gpuArray(w2);
embedding_g=gpuArray(embedding);
sampling_delay_g=gpuArray(sampling_delay);
p_ref_g=gpuArray(p_ref);
stps_g=gpuArray(stps);
delays_g=gpuArray(delays);
...
%             clear('rawdata_filtered')
            [x,y]=meshgrid(1:channels_len);
%             x=gpuArray(x);
%             y=gpuArray(y);
%             temp=arrayfun(@(x,y) SL_single_gpu(gpuArray(channels{x}),gpuArray(channels{y}),w1_g,w2_g,embedding_g,sampling_delay_g,p_ref_g,stps_g,delays_g),x,y,'UniformOutput',0);
%             SL{trial}=gather(temp);clear('temp')
            SL{trial}=arrayfun(@(x,y) SL_single_gpu((channels{x}),(channels{y}),w1,w2,embedding,sampling_delay,p_ref,stps,delays),x,y,'UniformOutput',0);
...
%                     
%                     t0=toc;
%                     SL{trial}{m,n}=SL_single(channels{m},channels{n},w1,w2,embedding,sampling_delay,p_ref,stps,delays);
%                     t1=toc-t0;
%                     
