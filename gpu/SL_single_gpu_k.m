function SL=SL_single_gpu_k(dataA,dataB,w1,w2,embedding,sampling_delay,p_ref,stps,delays,k)
...
%         vec_temp_B=cell(1,length(vref_range));
        vec_zeroA=zeros(stp_len,vec_num,embedding);
        vec_zeroB=zeros(stp_len,vec_num,embedding);
        for ref_idx=1:length(vref_range)
            ...
            vec_zeroA(ref_idx,:,:)=(vecA(:,tim)-repmat(vecA(:,ref_tim),1,(vec_num)))';
            vec_zeroB(ref_idx,:,:)=(vecB(:,tim)-repmat(vecB(:,ref_tim),1,(vec_num)))';
        end
        distA=zeros(stp_len*(vec_num),1);
        distB=zeros(stp_len*(vec_num),1);
        distA=feval(k,distA,reshape(vec_zeroA,1,[]),int32(embedding));
        distA=reshape(gather(distA),stp_len,(vec_num));
        distB=feval(k,distB,reshape(vec_zeroB,1,[]),int32(embedding));
        distB=reshape(gather(distB),stp_len,(vec_num));
        %% 根据p_ref计算关键距离
        ...
        SL(dly,:)=mean(synA_mat&synB_mat,2)'./p_ref;
