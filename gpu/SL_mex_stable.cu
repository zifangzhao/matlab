...
void __global__ SLGen(double * SL, int * pattern,const double * vref,double Nvec,double Nstep,double Nchn,double Nstart){
    int ref_idx=threadIdx.x; //vecs location,this can not be longer than 1024
    int tim_idx=blockIdx.y; //这个idx是startpoint和delay的混合一维，需要编码
...
//     int y=blockIdx.y;// channel number B
    ...
    int dly=tim_idx/(int)Nstart;
    int stp=tim_idx%(int)Nstart;
    int A=pattern[(int)(ref_idx+(stp)*Nvec+x*Nvec*Nstep)];
    int B=pattern[(int)(ref_idx+(stp+dly)*Nvec+y*Nvec*Nstep)];
    syn[2*ref_idx]=A&B;
    syn[1+2*ref_idx]=A+B;
    __syncthreads();
    if(ref_idx==0){
...
//     p_min_num=(double * )mxGPUGetData(min_num); 
    ...
    plhs[0]=mxCreateCellMatrix(Nchn+7,Nchn);
    //nlhs doesn't count?
...
//     mxSetCell(plhs[0],6,mxGPUCreateMxArrayOnCPU(pattern));  //testing by output to 1
    mxSetCell(plhs[0],7,mxGPUCreateMxArrayOnCPU(SL));  //testing by output to 1
