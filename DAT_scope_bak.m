...
global filename  hobj plotting_on  Yscale N_channel selected_channel time_scale fs LP_on HP_on WL_on Ref_on Mean_on HP_v LP_v WL_l WL_h WL_p HD32_on c_eff;
plotting_on=0;
...
global h3 h3a HD32_on plotting_on fs filename selected_channel Yscale N_channel time_scale LP_on HP_on WL_on Ref_on Mean_on LP_v HP_v WL_l WL_h WL_p first_plot WL_handles line_handles trig_handles c_eff current_beep
...
while plotting_on
    ...
    [a]=readmulti_frank(filename,N_channel,selected_channel,-display_time*fs,0,'int16' )';
    ff=dir(filename);
   ...
   std_str=num2str(std(a,[],2));
    ...
    for idx=1:length(selected_channel)
        ...
        if first_plot
            if ~WL_on
                hold(h,'off')
                line_handles(idx)=plot(h,xt,a(idx,1:res_t:end),'linewidth',1);
...
if get(handles.checkbox7,'Value')
    ...
    set(handles.Text_indicator,'String',[num2str(coordinates(1)*1000) ' Sec  ' num2str(coordinates(2)) ' Hz'])

else
   coordinates = get(axesHandle,'CurrentPoint'); 
   coordinates = coordinates(1,1:2);
   set(handles.Text_indicator,'String',[num2str(coordinates(1)*1000) ' Sec  ' num2str(coordinates(2)) ' uV'])
